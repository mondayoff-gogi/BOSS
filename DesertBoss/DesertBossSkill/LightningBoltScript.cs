...
namespace DigitalRuby.LightningBolt
{
    ...
    public class LightningBoltScript : MonoBehaviour
    {
        ...
        private BossStatus boss_stat;
        private bool is_destory = false;
        ...
        public void Trigger()
        {
            ...
            if (EndObject == null)
            {
                EndPosition = player[rand_num].transform.position;
                end = EndPosition;
            }
            else
            {
            ...
            if(player[rand_num].tag=="DeadPlayer"&&!is_destory) //전기 맞는중 사망, 죽는 경우
            {
                is_destory = true;
                this.GetComponent<SpriteRenderer>().material = Resources.Load("Material/Glitch") as Material;
                StartCoroutine(DestroyPillar());
            }
            else if(temp_timer<0) //전기맞는중 살아있는경우
            {
                temp_timer = 1; //1초마다 데미지
                if(player[rand_num].tag=="Player")  
                player[rand_num].GetComponent<CharacterStat>().GetDamage(player[rand_num].GetComponent<CharacterStat>().MaxHP*10f/100f, true);
            }

            if(Vector2.Distance(player[rand_num].transform.position,this.transform.position)>10 && !is_destory) //탑으로부터 멀리 가면 부서짐
            {
                is_destory = true;
                this.GetComponent<SpriteRenderer>().material = Resources.Load("Material/Glitch") as Material;
                StartCoroutine(DestroyPillar());
        ...
        IEnumerator DestroyPillar()
        {
            ...
            prefab_temp = Instantiate(prefab, this.gameObject.transform);
